...
const clients = [
  {
    ...
    clientSecret:
      process.env.CLIENT_SECRET || "39c4325efb1993344ff533646642d3fa",
    grants: ["authorization_code", "refresh_token"],
    redirectUris: [
      "https://zapier.com/dashboard/auth/oauth/return/App229953CLIAPI/",
    ],
  },
];
...
const model = {
  ...
  saveAuthorizationCode: async (code, client, user) => {
    const stored = {
      ...
      expiresAt:
        code.expiresAt || code.expirationDate || new Date(Date.now() + 600000),
  ...
  getAccessToken: async (accessToken) => {
    ...
    return (
      tokens.find(
        (t) => t.accessToken === accessToken || t.access_token === accessToken
      ) || null
    );
  },
  getRefreshToken: async (refreshToken) => {
    ...
    return (
      tokens.find(
        (t) =>
          t.refreshToken === refreshToken || t.refresh_token === refreshToken
      ) || null
    );
  },
  ...
  verifyScope: async (token, scope) => {
    console.log("verifyScope ->", {
      token: token && (token.accessToken || token.access_token),
      scope,
    });
    return true;
  },
...
app.get("/oauth/authorize", (req, res) => {
  ...
  oauth
    .authorize(request, response, {
      authenticateHandler: {
        handle: (reqInner) => {
          ...
          return reqInner.session && reqInner.session.user
            ? reqInner.session.user
            : null;
        },
      },
    })
    .then((code) => {
      ...
      res.redirect(
        `${redirectUri}?code=${authorizationCode}&state=${encodeURIComponent(
          state
        )}`
      );
    })
    .catch((err) => {
      ...
      res
        .status(err.code || 400)
        .json({ error: err.message || "authorize_error" });
    });
});
    ...
    .then((token) => {
      ...
      const expires_in =
        ...
        (out.accessTokenExpiresAt
          ? Math.floor((new Date(out.accessTokenExpiresAt) - Date.now()) / 1000)
          : 3600);
...
app.get("/api/me", (req, res) => {
  console.log(
    "GET /api/me — Authorization header:",
  ...
  oauth
    ...
    .then((token) => {
      console.log(
        "/api/me authenticated token:",
        token && (token.accessToken || token.access_token)
      );
      res.json({
    ...
    .catch((err) => {
      ...
      res
        ...
        .json({
          error:
            "Unauthorized request: no authentication given or token invalid",
        });
    });
});
...
app.post("/api/submissions", (req, res, next) => {
  console.log("POST /api/submissions called:", req.body);
  const request = new OAuth2Server.Request(req);
  const response = new OAuth2Server.Response(res);
  oauth
    .authenticate(request, response)
    .then(() => {
      ...
      console.log("Creating submission:", { name, email, message });
      if (!name || !email) {
...
app.use((req, res) => {
  ...
  // return JSON so Zapier shows structured error instead of HTML
  res
    .status(404)
    .json({ error: "not_found", path: req.originalUrl, method: req.method });
});
